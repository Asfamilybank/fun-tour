  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <Controller
        ...
        render={({ field, fieldState: { error } }) => (
          <div className="form-control">
            <input {...field} className={`input input-bordered ${error ? 'input-error' : ''}`} placeholder="请输入昵称" tabIndex={1} autoFocus />
        ...
        render={({ field, fieldState: { error } }) => (
          <div className="form-control">
            <div className="input-group">
              ...
              <input {...field} className={`input input-bordered grow ${error ? 'input-error' : ''}`} placeholder="请输入账户名" tabIndex={2} />
              <Controller
                control={control}
                name="type"
                render={({ field: field2 }) => (
      ...
      <Controller
        ...
        render={({ field, fieldState: { error } }) => (
          <div className="form-control">
            <input className={`input input-bordered ${error ? 'input-error' : ''}`} placeholder="请再次确认密码" tabIndex={2} type="password" {...field} />
            <label className="label" ref={AnimateForm}>
              {error && <span className="label-text-alt text-error">{error.message}</span>}
            </label>
          </div>
