...
) => Promise<FailResponse | Response<T>>

const useRequest = <T = any, D = any>(
  url: string,
  post: PostType,
  ...
  const useMySWR = (
    params?: D,
    config?: SWRConfiguration<IRequestResponse<T>>,
    isRequest = true
  ) => {
    const {
      data: response,
      ...
      isValidating,
      mutate
    } = useSWR<IRequestResponse<T>>(
      isRequest ? [url, params, options] : null,
      post,
      config
    )
